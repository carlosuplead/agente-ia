-- Várias ferramentas N8N por workspace (JSONB). Mantém colunas legadas sincronizadas pela API.
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS n8n_tools JSONB NOT NULL DEFAULT ''[]''::jsonb',
            tenant_slug
        );

        EXECUTE format(
            'UPDATE %I.ai_agent_config SET n8n_tools = jsonb_build_array(
                jsonb_build_object(
                    ''tool_name'', ''call_n8n_webhook'',
                    ''url'', n8n_webhook_url,
                    ''timeout_seconds'', n8n_webhook_timeout_seconds,
                    ''description'', COALESCE(NULLIF(btrim(n8n_tool_description), ''''), '''')
                )
            )
            WHERE coalesce(n8n_webhook_enabled, false) = true
              AND n8n_webhook_url IS NOT NULL
              AND btrim(n8n_webhook_url) <> ''''
              AND n8n_tools = ''[]''::jsonb',
            tenant_slug
        );
    END LOOP;
