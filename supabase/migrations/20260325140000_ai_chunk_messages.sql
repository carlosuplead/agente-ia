-- Resposta IA em várias bolhas WhatsApp (opcional): split por parágrafo ou linha.
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_chunk_messages_enabled BOOLEAN NOT NULL DEFAULT FALSE',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_chunk_split_mode TEXT NOT NULL DEFAULT ''paragraph''',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_chunk_max_parts INTEGER NOT NULL DEFAULT 8',
            tenant_slug
        );
    END LOOP;
END;
