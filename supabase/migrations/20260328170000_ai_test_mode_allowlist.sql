-- Modo testes: allowlist de telefones (ai_agent_config)
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_test_mode BOOLEAN NOT NULL DEFAULT FALSE',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_test_allowlist_phones TEXT',
            tenant_slug
        );
    END LOOP;
END;
