-- Tool notify_team_whatsapp: destinatários autorizados e opções (ai_agent_config)
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS team_notification_enabled BOOLEAN NOT NULL DEFAULT FALSE',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS team_notification_allowlist_phones TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS team_notification_tool_description TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS team_notification_append_transcript BOOLEAN NOT NULL DEFAULT TRUE',
            tenant_slug
        );
    END LOOP;
END;
