-- Notificação ao vendedor via UAZAPI separada (quando agenda é criada).
-- O cliente configura uma instância UAZAPI dedicada (URL base + token) e os
-- telefones a notificar. O agente dispara automaticamente após a criação do
-- evento no Google Calendar. Campos por tenant em ai_agent_config.
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS seller_notification_enabled BOOLEAN NOT NULL DEFAULT FALSE',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS seller_notification_uazapi_url TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS seller_notification_uazapi_token TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS seller_notification_phones TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS seller_notification_on_appointment BOOLEAN NOT NULL DEFAULT TRUE',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS seller_notification_message_template TEXT',
            tenant_slug
        );
    END LOOP;
END;
...
-- Atualiza a função de criação de novos tenants para incluir as novas colunas.
