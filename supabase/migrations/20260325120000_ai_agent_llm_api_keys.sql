-- Chaves LLM opcionais por workspace (BYOK); fallback: OPENAI_API_KEY / GOOGLE_API_KEY no servidor.
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS openai_api_key TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS google_api_key TEXT',
            tenant_slug
        );
    END LOOP;
END;
