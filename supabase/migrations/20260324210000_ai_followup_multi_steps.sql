-- Vários passos de follow-up (JSONB). Âncora + progresso na conversa; remove colunas legadas de um só passo.
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_followup_steps JSONB NOT NULL DEFAULT ''[]''::jsonb',
            tenant_slug
        );
        ...
        IF EXISTS (
            ...
            WHERE c.table_schema = tenant_slug
              AND c.table_name = 'ai_agent_config'
              AND c.column_name = 'ai_followup_message'
        ) THEN
            EXECUTE format(
                'UPDATE %I.ai_agent_config SET ai_followup_steps = jsonb_build_array(
                    jsonb_build_object(
                        ''delay_minutes'',
                        GREATEST(1, COALESCE(ai_followup_after_hours, 24) * 60),
                        ''message'',
                        ai_followup_message
                    )
                )
                WHERE jsonb_array_length(ai_followup_steps) = 0
                  AND ai_followup_message IS NOT NULL
                  AND btrim(ai_followup_message) <> ''''',
                tenant_slug
            );
        END IF;

        EXECUTE format(
            'ALTER TABLE %I.ai_conversations ADD COLUMN IF NOT EXISTS ai_followup_anchor_at TIMESTAMPTZ',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_conversations ADD COLUMN IF NOT EXISTS ai_followup_progress INTEGER NOT NULL DEFAULT 0',
            tenant_slug
        );

        EXECUTE format(
            'UPDATE %I.ai_conversations SET ai_followup_progress = 1 WHERE ai_followup_sent_at IS NOT NULL',
            tenant_slug
        );

        EXECUTE format(
            'ALTER TABLE %I.ai_conversations DROP COLUMN IF EXISTS ai_followup_sent_at',
            tenant_slug
        );

        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config DROP COLUMN IF EXISTS ai_followup_after_hours',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config DROP COLUMN IF EXISTS ai_followup_message',
            tenant_slug
        );
    END LOOP;
END;
