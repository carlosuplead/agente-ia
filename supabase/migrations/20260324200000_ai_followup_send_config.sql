-- Follow-up automático: texto + horas; marca por conversa para não repetir até o cliente responder.
...
BEGIN
    FOR tenant_slug IN SELECT slug FROM public.workspaces LOOP
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_followup_after_hours INTEGER NOT NULL DEFAULT 24',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_agent_config ADD COLUMN IF NOT EXISTS ai_followup_message TEXT',
            tenant_slug
        );
        EXECUTE format(
            'ALTER TABLE %I.ai_conversations ADD COLUMN IF NOT EXISTS ai_followup_sent_at TIMESTAMPTZ',
            tenant_slug
        );
    END LOOP;
END;
...
BEGIN
    ...
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.ai_agent_config (
            ...
            ai_followup_after_hours INTEGER NOT NULL DEFAULT 24,
